...
<head>
    ...
    <title>Invoice_{{ str_replace("DO","INV",$master->sales_delivery_id ?? '') }}</title>
    ...
    <div id='page'>
        <div>
            ...
            <div id="box">
                <h1>
                    <span>
                        NO. {{ str_replace("DO","INV",$master->sales_delivery_id ?? '') }}
                    </span>
                </h1>
                <h2>
                    Order Date : {{ $master->order->sales_order_date_order->format('d F Y') }}
                </h2>
                <table>
                    <tr>
                        <td class="head">
                        ...
                        <td>
                            {{ $master->order->sales_order_from_name ?? '' }} (
                            {{ $master->order->sales_order_from_phone ?? '' }} )
                        </td>
                    </tr>
                    <tr>
                        <td class="head">
                        ...
                        <td>
                            {{ $master->order->customer->crm_customer_invoice_person ?? '' }}
                        </td>
                    </tr>
                </table>
            </div>
        ...
        <div id="address">
            <h4>
                {{ $master->order->sales_order_from_address }}
            </h4>
            <p>
                {{ Helper::getSingleArea($master->order->sales_order_from_area, true) }}
            </p>
        </div>
        <div id="container">
            <table cellpadding="" 5 cellspacing="0" width="100%">
                ...
                <tr class="contact">
                    <td colspan='8'>
                        <strong>
                            {{ $master->order->customer->crm_customer_name ?? '' }}
                        </strong>
                        <p>
                            {{ $master->order->sales_order_to_address ?? '' }} /
                            {{ Helper::getSingleArea($master->order->sales_order_to_area, true) ?? '' }}
                        </p>
                    </td>
                </tr>
                <tr>
                    <td colspan="8">
                        <p>
                            @php
                            $total = $detail->sum('sales_delivery_detail_total');
                            $discount = $master->order->sales_order_discount_percent * $total / 100;
                            $tax = $total * $master->order->sales_order_tax_percent / 100;
                            $grand = intval($total - $discount) - intval($tax);
                            @endphp
                            <strong style="font-size: 12px;">
                                Total Amount : Rp {{ Helper::createRupiah($grand) }} ( <span style="font-style: italic;">{{ Helper::terbilang($grand) }} rupiah. </span>)
                            </strong>
                        </p>
                    </td>
                </tr>
                ...
                <tr class="item">
                    ...
                    <td class="qty">
                        {{ $item->sales_delivery_detail_out ?? '' }}
                    </td>
                    <td class="total">
                ...
                <tr class="total_product">
                    ...
                    <td class="qty">
                        {{ $detail->sum('sales_delivery_detail_out') ?? '' }}
                    </td>
                    <td class="total">
                ...
                @if (!empty($master->order->sales_order_discount_percent))
                <tr class="total_discount">
                    <td class="product" colspan="6">
                        {{ $master->order->sales_order_discount_name ?? '' }}
                        Total Discount
                    </td>
                    <td class="qty">
                        {{ $master->order->sales_order_discount_percent ?? '' }}%
                    </td>
                    <td class="total">
                        -{{ Helper::createRupiah($discount) ?? '' }}
                    </td>
                </tr>
                @endif
                @if (!empty($master->order->sales_order_tax_percent))
                <tr class="total_discount">
                    <td class="product" colspan="6">
                        {{ $master->order->tax->finance_tax_name ?? '' }}
                    </td>
                    <td class="qty">
                        {{ $master->order->sales_order_tax_percent ?? '' }}%
                    </td>
                    <td class="total">
                        {{ Helper::createRupiah($tax) ?? '' }}
                    </td>
                </tr>
                @endif
        ...
        <table width="100%" border="0" style="margin-top:10px;width:100% !important;font-size:10px;margin-bottom:-50px">
            <tr>
                <td align="center">{{ $master->order->customer->crm_customer_name ?? '' }}</td>
