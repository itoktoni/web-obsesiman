...
class BenefitController extends Controller
{
    ...
    {
        if (self::$model == null) {
            self::$model = new BenefitRepository();
        }
    ...
    {
        if (request()->isMethod('POST')) {
            $datatable = $service->setRaw(['marketing_benefit_image'])->datatable(self::$model);
            $datatable->editColumn('marketing_benefit_image', function ($select) {
                return Helper::createImage(Helper::getTemplate(__CLASS__) . '/thumbnail_' . $select->marketing_benefit_image);
            });
