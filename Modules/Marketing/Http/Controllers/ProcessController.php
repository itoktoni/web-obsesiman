...
class ProcessController extends Controller
{
    ...
    {
        if (self::$model == null) {
            self::$model = new ProcessRepository();
        }
    ...
    {
        if (request()->isMethod('POST')) {
            $datatable = $service->setRaw(['marketing_process_image'])->datatable(self::$model);
            $datatable->editColumn('marketing_process_image', function ($select) {
                return Helper::createImage(Helper::getTemplate(__CLASS__) . '/thumbnail_' . $select->marketing_process_image);
            });
