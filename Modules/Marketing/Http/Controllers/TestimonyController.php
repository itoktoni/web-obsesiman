...
class TestimonyController extends Controller
{
    ...
    {
        if (self::$model == null) {
            self::$model = new TestimonyRepository();
        }
    ...
    {
        if (request()->isMethod('POST')) {
            $datatable = $service->setRaw(['marketing_testimony_image'])->datatable(self::$model);
            $datatable->editColumn('marketing_testimony_image', function ($select) {
                return Helper::createImage(Helper::getTemplate(__CLASS__) . '/thumbnail_' . $select->marketing_testimony_image);
            });
