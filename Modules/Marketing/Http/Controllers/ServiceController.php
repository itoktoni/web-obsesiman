...
class ServiceController extends Controller
{
    ...
    {
        if (self::$model == null) {
            self::$model = new ServiceRepository();
        }
    ...
    {
        if (request()->isMethod('POST')) {
            $datatable = $service->setRaw(['marketing_service_image'])->datatable(self::$model);
            $datatable->editColumn('marketing_service_image', function ($select) {
                return Helper::createImage(Helper::getTemplate(__CLASS__) . '/thumbnail_' . $select->marketing_service_image);
            });
